...
@section('title', 'Manajemen Kalendar Event')
...
@section('page-script')
<script src="{{asset('assets/js/tables_list_event_bc.js')}}"></script>
@endsection
...
<h4 class="py-3 breadcrumb-wrapper mb-4">
  <span class="">Manajemen Kalender Event</span>
</h4>
...
<script>
function gantiStatus(obj){
	var selectedValue = obj.value;

	let text = obj.id;
	const myArray = text.split("-");
	var selectedId = myArray[1];

	// alert("val="+selectedValue+",id="+selectedId);
	
	$.ajax({
		url: '/edit-status-calendar',
		type: "get",
		data: {id:selectedId,ca:selectedValue},
		success: function (data) {
			if(data == "failed"){
				Swal.fire(
					'Gagal!',
					'Status kalender gagal diubah.',
					'error'
				);
			}else{
				Swal.fire(
					'Berhasil!',
					'Status kalender berhasil diubah.',
					'success'
				);
			}
		}
...
<script>
	function model_show_chart(nmr_event){
		...
		$.getJSON("/bbm/get_grafik_ach_event?nmr_event="+nmr_event, function(data){
